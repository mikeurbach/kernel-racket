...
module attributes {tf.versions = {bad_consumers = [], min_consumer = 12 : i32, producer = 175 : i32}, tf_saved_model.semantics} {
  func @__inference_gcd_360(%arg0: tensor<i32> {tf_saved_model.index_path = [0]}, %arg1: tensor<i32> {tf_saved_model.index_path = [1]}) -> (tensor<i32> {tf_saved_model.index_path = []}) attributes {tf._input_shapes = ["tfshape$", "tfshape$"], tf_saved_model.exported_names = ["gcd"]} {
    %0 = tf_executor.graph {
      ...
      %outputs_4, %control_5 = tf_executor.island wraps "tf.StopGradient"(%outputs_2#2) {T = i32, _output_shapes = ["tfshape$"], device = ""} : (tensor<i32>) -> tensor<i32>
      %outputs_6, %control_7 = tf_executor.island wraps "tf.Identity"(%outputs_4) {T = i32, _output_shapes = ["tfshape$"], device = ""} : (tensor<i32>) -> tensor<i32>
      %outputs_8, %control_9 = tf_executor.island wraps "tf.StopGradient"(%outputs_2#3) {T = i32, _output_shapes = ["tfshape$"], device = ""} : (tensor<i32>) -> tensor<i32>
      tf_executor.fetch %outputs_6 : tensor<i32>
    }
    return %0 : tensor<i32>
  }
  func @while_body_60(%arg0: tensor<i32>, %arg1: tensor<i32>, %arg2: tensor<i32>, %arg3: tensor<i32>) -> (tensor<i32>, tensor<i32>, tensor<i32>, tensor<i32>) attributes {tf._input_shapes = ["tfshape$", "tfshape$", "tfshape$", "tfshape$"]} {
    %0:4 = tf_executor.graph {
      ...
      %outputs_0, %control_1 = tf_executor.island wraps "tf.FloorMod"(%arg2, %arg3) {T = i32, _output_shapes = ["tfshape$"], device = ""} : (tensor<i32>, tensor<i32>) -> tensor<i32>
      %outputs_2, %control_3 = tf_executor.island wraps "tf.Identity"(%outputs_0) {T = i32, _output_shapes = ["tfshape$"], device = ""} : (tensor<i32>) -> tensor<i32>
      %outputs_4, %control_5 = tf_executor.island wraps "tf.Identity"(%arg3) {T = i32, _output_shapes = ["tfshape$"], device = ""} : (tensor<i32>) -> tensor<i32>
      ...
      tf_executor.fetch %outputs_8, %outputs_10, %outputs_4, %outputs_2 : tensor<i32>, tensor<i32>, tensor<i32>, tensor<i32>
    }
