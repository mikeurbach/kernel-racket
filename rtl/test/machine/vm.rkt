#lang racket

(require
 (only-in kernel global-env)
 "../../../kernel/src/core/combiner.rkt"
 "../../../kernel/src/core/environment.rkt"
 "../../src/machine/vm.rkt"
 rackunit)

(define test-environment (make-environment (list global-env)))
(bind! test-environment '+ (make-applicative +))
(bind! test-environment '- (make-applicative -))
(bind! test-environment '* (make-applicative *))
(bind! test-environment '< (make-applicative <))
(bind! test-environment 'remainder (make-applicative remainder))

(define (run-vm insts)
  (define myvm (new vm [environment test-environment] [instructions insts]))
  (send myvm execute)
  myvm)

(define (assert-register myvm name value)
  (let ([register-value (send myvm vm-get-register-value name)])
    (check-equal? register-value value)))

(let ([myvm (run-vm '((assign ((reg foo) (const 420)))))])
  (assert-register myvm 'foo 420))

(let ([myvm (run-vm '((assign ((reg foo) (const 420)))
                      (assign ((reg foo) (const 69)))))])
  (assert-register myvm 'foo 69))

(let ([myvm (run-vm '((assign ((reg foo) (const 420)))
                      (assign ((reg bar) (reg foo)))))])
  (assert-register myvm 'foo 420)
  (assert-register myvm 'bar 420))

(let ([myvm (run-vm '((assign ((reg foo) (const 420)))
                      (assign ((reg bar) (const 69)))
                      (assign ((reg baz) (op +) (reg foo) (reg bar)))))])
  (assert-register myvm 'foo 420)
  (assert-register myvm 'bar 69)
  (assert-register myvm 'baz 489))

(let ([myvm (run-vm '((assign ((reg val) (const 1)))
                      (branch (#t (const done)))
                      (assign ((reg val) (const 2)))
                      done))])
  (assert-register myvm 'val 1))

(let ([myvm (run-vm '((assign ((reg val) (const 1)))
                      loop
                      (assign ((reg val) (op +) (reg val) (const 1)))
                      (branch (((op <) (reg val) (const 10)) (const loop)))))])
  (assert-register myvm 'val 10))

(let ([myvm (run-vm '((assign ((reg val) (const 1)))
                      loop
                      (assign ((reg val) (op +) (reg val) (const 1)))
                      (branch (((op eq?) (reg val) (const 5)) (const done))
                              (((op <) (reg val) (const 10)) (const loop)))
                      done))])
  (assert-register myvm 'val 5))

(let ([myvm (run-vm '((assign ((reg a) (const 1))
                              ((reg b) (reg a)))))])
  (assert-register myvm 'a 1)
  (assert-register myvm 'b #f))

(let ([myvm (run-vm '((assign ((reg a) (const 420))
                              ((reg b) (const 69)))
                      (assign ((reg a) (reg b))
                              ((reg b) (reg a)))))])
  (assert-register myvm 'a 69)
  (assert-register myvm 'b 420))

;; branch from register
(let ([myvm (run-vm '((assign ((reg label-reg) (const a)))
                      (branch (#t (reg label-reg)))
                      (assign ((reg val) (const wrong)))
                      (branch (#t (const done)))
                      a
                      (assign ((reg val) (const right)))
                      done))])
  (assert-register myvm 'val 'right))

;; register aliasing for read
(let ([myvm (run-vm '((assign ((reg foo) (const 1))
                              ((reg myalias) (const foo)))
                      (assign ((reg result) (alias myalias)))))])
  (assert-register myvm 'result 1))

;; register aliasing for write
(let ([myvm (run-vm '((assign ((reg foo) (const 1))
                              ((reg myalias) (const foo)))
                      (assign ((alias myalias) (const 2)))))])
  (assert-register myvm 'foo 2))

;; GCD
(let ([myvm (new vm
                 [environment test-environment]
                 [instructions '(test-b
                                 (branch (((op eq?) (reg b) (const 0)) (const done)))
                                 (assign ((reg a) (reg b))
                                         ((reg b) (op remainder) (reg a) (reg b)))
                                 (branch (#t (const test-b)))
                                 done
                                 (assign ((reg result) (reg a))))])])
  (send myvm vm-set-register-value! 'a 24)
  (send myvm vm-set-register-value! 'b 9)
  (send myvm execute)
  (assert-register myvm 'result 3))

;; Fibonacci
(let ([myvm (new vm
                 [environment test-environment]
                 [instructions '((assign ((reg prev) (const 0))
                                         ((reg curr) (const 1)))
                                 test-n
                                 (branch (((op eq?) (reg n) (const 0)) (const done)))
                                 (assign ((reg curr) (op +) (reg prev) (reg curr))
                                         ((reg prev) (reg curr))
                                         ((reg n) (op -) (reg n) (const 1)))
                                 (branch (#t (const test-n)))
                                 done
                                 (assign ((reg result) (reg prev))))])])
  (send myvm vm-set-register-value! 'n 100)
  (send myvm execute)
  (assert-register myvm 'result 354224848179261915075))

;; Factorial
(let ([myvm (new vm
                 [environment test-environment]
                 [instructions '((assign ((reg product) (const 1)))
                                 test-n
                                 (branch (((op eq?) (reg n) (const 0)) (const done)))
                                 (assign ((reg product) (op *) (reg n) (reg product))
                                         ((reg n) (op -) (reg n) (const 1)))
                                 (branch (#t (const test-n)))
                                 done
                                 (assign ((reg result) (reg product))))])])
  (send myvm vm-set-register-value! 'n 5)
  (send myvm execute)
  (assert-register myvm 'result 120))
